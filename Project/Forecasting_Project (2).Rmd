...
## Task 2:
  ...
  Here, we will forecast the First Flowering day data for the next four years using the best fit model. To get this best model we have three approaches.            
...
Comparitively, based on AIC, BIC and MASE scores ets(M, M, N) is better.

Among all the methods, ses in simple smoothing.

## Forecasting

Let us forecast the mortality rate for the next 4 weeks using the best model.

### Forcasting with Smoothing method best model

```{r}
fit <- ses(v_Mortality_data_TS, seasonal = "simple", h = 4)

v_mort_forecasts <- ts.intersect(ts(fit$lower[, 2], start = c(2020), frequency = 356.27/7), ts(fit$mean, start = c(2020), frequency = 356.27/7), ts(fit$upper[, 2], start = c(2020), frequency = 356.27/7))
colnames(v_mort_forecasts) <- c("Lower bound", "Point forecast", "Upper bound")
...
legend("top", inset = .03, cex = 0.9, box.lty = 2, box.lwd = 2, pch = 1, lty = 1, col = c("red", "blue"), c("Data", "Forecasts"))
```

Fig 1.19: Next 4 weeks forecast on the Mortality rate Series with Smoothing method model.

From the four weeks forecast results we can predict that there will be decrease in the mortality rate in the future.

### Forcasting with ets method best model

```{r}
fit3 <- ets(v_Mortality_data_TS, model="MMN", damped = T)
fit <- forecast.ets(fit3, h = 4)
...
Fig 1.20: Next 4 weeks forecast on the Mortality rate Series with ets method model.
...
```{r}
fit4 <- holt(v_First_Flowering_Day_data_TS, damped = TRUE, h = 4)# Fit with damped trend
summary(fit4)
```
...
```{r}
v_ets_fit <- ets(v_First_Flowering_Day_data_TS)
summary(v_ets_fit)
```
...
```{r Fore2}
fit <- holt(v_First_Flowering_Day_data_TS, damped = TRUE, h = 4)
...
Fig 2.21: Next 4 year forecast on the First Flowering Day Series with Smoothing method model.

From the four year forecast results we can predict that there will be decrease in the First Flowering Day in the future. This suggests that the impact of the chemical components will be less in future.

### Forcasting with ets method best model

```{r}
fit3 <- ets(v_First_Flowering_Day_data_TS, model="MMN", damped = T)
fit1 <- forecast.ets(fit3, h = 4)
plot(fit1, fcol = "white", main = "First Flowerind Day series with 4 years ahead forecasts", ylab = "Radiation")
...
```{r}
forecasts <- ts.intersect(ts(fit1$lower[, 2], start = c(2015), frequency = 1), ts(fit1$mean, start = c(2015), frequency = 1), ts(fit1$upper[, 2], start = c(2015), frequency = 1))
colnames(forecasts) <- c("Lower bound", "Point forecast", "Upper bound")
forecasts
```

Fig 2.22: Next 4 year forecast on the First Flowering Day Series with ets method model.
...
```{r}
fit3 <- ets(v_RBO_data_TS, model="MMN", damped = T)
fit1 <- forecast.ets(fit3, h = 3)
```
...
Among all the polynomial model with temperature has the least MASE score compared.
...
```{r}
plot(fit1, fcol = "white", main = "First Flowerind Day series with 3 years ahead forecasts", ylab = "Radiation")
lines(fitted(fit1), col = "darkgreen")
lines(fit1$mean, col = "darkgreen", lwd = 2)
legend("topleft", lty = 1, col = c("black", "darkgreen"), c("Data", "Forecasts"))
```
...
From the three year forecast results we can predict that there will be decrease in the Rank-based Order similarity metric in the future. This suggests that the impact of the chemical components will be less in future.
...
But the MASE scores are shown least for simple seasonal smoothing method.
Also, ets(M, M, N) is better in state space variations method.

So we considered these as the best model for forecasting the Mortality rate data for the next 4 weeks. From the 4 weeks forecast results we can predict that there will be decrease in the Mortality rate in the future. This suggests that the impact of the chemical components will be leess in future. But as we forecast with 95% confidence intervals we cannot consider this as accurate.
...
Among all the methods, holt in exponential smoothing with damped trend. When compared with MASE, BIC and AIC score is better in Smoothing method.
Also, ets(M, M, N) is better in state space variations method.

So we considered these as the best model for forecasting the First Flower Day data for the next 4 years. From the 4 years forecast results we can predict that there will be decrease in the First Flower Day in the future. This suggests that the impact of the chemical components will be less in future. But as we forecast with 95% confidence intervals we cannot consider this as accurate.
